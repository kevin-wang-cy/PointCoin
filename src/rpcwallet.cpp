...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "Returns a new pointcoin address for receiving payments.\n");
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current pointcoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <pointcoinaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid pointcoin address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 7)
        throw runtime_error(
            "sendtoaddress <pointcoinaddress> <amount> [comment] [comment-to] [tx-comment] [product-id] [narration]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <pointcoinaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <pointcoinaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "getreceivedbyaddress <pointcoinaddress> [minconf=1] [multisig=false]\n"
            "Returns the total amount received by <pointcoinaddress> in transactions with at least [minconf] confirmations.\n"
...
Value getreceivedbyaccount(const Array& params, bool fHelp)
{
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, wtx.vout)
        {
            ...
            if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*pwalletMain, address, fMultiSig) && setAddress.count(address))
                if (wtx.GetDepthInMainChain() >= nMinDepth)
                {
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <topointcoinaddress> <amount> [minconf=1] [comment] [comment-to] [tx-comment] [product-id] [narration]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
            "[tx-comment] max size is " + strprintf("%d", MAX_TX_COMMENT_LEN) + "\n"
            "[product-id] is a positive integer\n"
	    "[narration] is a short (<=24 char) note"
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    int nColor = (int) PNTS_COLOR_NONE;
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid pointcoin address: ")+s.name_);

        if (nColor == (int) PNTS_COLOR_NONE)
        {
...
void ListTransactions(const CWalletTx& wtx, const string& strAccount,
                      int nMinDepth, bool fLong, Array& ret, bool fMultiSig)
{
    int64_t nFee;
    string strSentAccount;
    list<pair<CTxDestination, int64_t> > listReceived;
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("pointcoin-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("pointcoin-lock-wa");
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; pointcoin server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            "validateaddress <pointcoinaddress> [ticker]\n"
            "Return information about <pointcoinaddress>.\n"
    ...
    CBitcoinAddress address(params[0].get_str());
    bool isValid;
    if ((nColor != (int) PNTS_COLOR_NONE) && (address.nColor != nColor))
    {
       isValid = false;
    }
    else
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <pointcoinpubkey> [ticker]\n"
            "Return information about <pointcoinpubkey>.");
    ...
    if (isValid)
    {
        CTxDestination dest = address.Get();
        string currentAddress = address.ToString();
        if (address.nColor == PNTS_COLOR_NONE)
        {
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid PointCoin Stealth Address."));
        return result;
    };
