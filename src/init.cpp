...
void Shutdown(void* parg)
{
    ...
    RenameThread("pointcoin-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("pointcoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  pointcoind [options]                     " + "\n" +
                  "  pointcoind [options] <command> [params]  " + _("Send command to -server or pointcoind") + "\n" +
                  "  pointcoind [options] help                " + _("List commands") + "\n" +
                  "  pointcoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "pointcoin:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("pointcoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("pointcoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: pointcoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: pointcoind.pid)") + "\n" +
        ...
        "  -defaultcurrency=<ticker>   " + _("Default: XPP") + "\n" +
        "  -defaultstake=<ticker>   " + _("Default: XPP") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    assert ((int) PNTS_COLOR_END == N_COLORS);
    ...
    // set the number of staking currencies
    {
        ...
        for (int nColor = 1; nColor < N_COLORS; ++nColor)
        {
             ...
             if (nMintColor != (int) PNTS_COLOR_NONE)
             {
    ...
    else if ((nDefaultStake == PNTS_COLOR_NONE) && (nNumberOfStakingCurrencies > 1))
    {
        std::string strDefaultStake = GetArg("-defaultstake", COLOR_TICKER[PNTS_COLOR_NONE]);
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  pointcoin is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "pointcoin server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("pointcoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
...
/* Reserve Balances */
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        {
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of pointcoin") << "\n";
        }
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart pointcoin to complete") << "\n";
