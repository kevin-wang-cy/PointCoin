...
const string strMessageMagic = "PointCoin Signed Message:\n";

// Settings

// nDefaultCurrency and nDefaultStake are for sideways compatibility.
int nDefaultCurrency = (int) PNTS_COLOR_NONE;
...
int nDefaultStake = (int) PNTS_COLOR_NONE;
...
struct AMOUNT GetPoWSubsidy(int nHeight)
{
    ...
    stSubsidy.nColor = (int) PNTS_COLOR_XPP;
    ...
    else if (nHeight <= nLastPoWBlock)
    {
        ...
        // total will be 1152
        stSubsidy.nValue = COIN[stSubsidy.nColor] / 5;
    }
...
unsigned int PointCoinGravityWave(const CBlockIndex* pindexLast, bool fProofOfStake)
{
...
{
    return PointCoinGravityWave(pindexLast, fProofOfStake);
}
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    int nStakeColor = (int) PNTS_COLOR_NONE;

    // *claimed* subsidy from GetProofOfStakeReward()
    int64_t nMintValue = 0;
    int nMintColor = (int) PNTS_COLOR_NONE;
    ...
    if (IsProofOfWork())
    {
        ...
        pindex->nStakeColor = (int) PNTS_COLOR_NONE;
    ...
    if (IsProofOfStake())
    {
        ...
        // pointcoin: reward based nHeight and color
        struct AMOUNT reward = GetProofOfStakeReward(pindex->pprev);

        // pointcoin: reward is earned in the mint transaction, not the stake transaction
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PointCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Trump Widens Rift With Russia Experts Over Hacking";
        CTransaction txNew;
        txNew.nVersion = 1;
        txNew.nTime = 1481450400;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.strTxComment = "";
        ...
        txNew.SetColor((int) PNTS_COLOR_NONE);
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1481450400;
        ...
        assert(block.hashMerkleRoot ==
          uint256("0x6a457dd5b28d3dbc3105c70200ca749061129397943973081d0672ff41ca87cf"));
