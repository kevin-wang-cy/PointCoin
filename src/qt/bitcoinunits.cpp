                                           ...
                                           bool fPlus, bool localized)
{
    ...
    // instead of 8, make it log_10(COIN)
    QString remainder_str = QString::number(remainder).rightJustified(8, '0');
    // trim leading zeros
    remainder_str.chop(8 - num_decimals);
...
bool BitcoinUnits::parse(int unit, const QString &value, int nColorIn, qint64 *val_out)
{
    ...
    bool ok = false;
    // TODO: not left justified 8, but log_10(COIN)
    QString str = whole + decimals.leftJustified(8, '0');
    if (!ok)
    {
        if (fDebug)
        {
