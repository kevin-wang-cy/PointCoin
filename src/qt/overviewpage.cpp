...
{
    int unitPointCoin = model->getOptionsModel()->getDisplayUnitPointCoin();
    ...
    ui->labelBalancePointCoin->setText(BitcoinUnits::formatWithUnitLocalized(
             unitPointCoin, mapSpendable[PNTS_COLOR_XPP], PNTS_COLOR_XPP));
    ui->labelStakePointCoin->setText(BitcoinUnits::formatWithUnitLocalized(
             unitPointCoin, mapCurrentStake[PNTS_COLOR_XPP], PNTS_COLOR_XPP));
    ui->labelUnconfirmedPointCoin->setText(BitcoinUnits::formatWithUnitLocalized(
             unitPointCoin, mapCurrentUnconfirmedBalance[PNTS_COLOR_XPP], PNTS_COLOR_XPP));
    ui->labelImmaturePointCoin->setText(BitcoinUnits::formatWithUnitLocalized(
             unitPointCoin, mapCurrentImmatureBalance[PNTS_COLOR_XPP], PNTS_COLOR_XPP));
    ui->labelTotalPointCoin->setText(BitcoinUnits::formatWithUnitLocalized(
             unitPointCoin, mapSpendable[PNTS_COLOR_XPP] +
                           mapCurrentStake[PNTS_COLOR_XPP] +
                           mapCurrentUnconfirmedBalance[PNTS_COLOR_XPP], PNTS_COLOR_XPP));

    // only show unconfirmed, stake, immature balances non-zero, so as not to complicate things
    bool showUnconfirmedPointCoin = mapCurrentUnconfirmedBalance[PNTS_COLOR_XPP] != 0;
    ui->labelUnconfirmedPointCoin->setVisible(showUnconfirmedPointCoin);
    ui->lblUniUn->setVisible(showUnconfirmedPointCoin);
    bool showStakePointCoin = mapCurrentStake[PNTS_COLOR_XPP] != 0;
    ui->labelStakePointCoin->setVisible(showStakePointCoin);
    ui->lblUniStk->setVisible(showStakePointCoin);
    bool showImmaturePointCoin = mapCurrentImmatureBalance[PNTS_COLOR_XPP] != 0;
    ui->labelImmaturePointCoin->setVisible(showImmaturePointCoin);
    ui->lblUniImm->setVisible(showImmaturePointCoin);
...
void OverviewPage::setModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        connect(model->getOptionsModel(), SIGNAL(displayUnitChangedPointCoin(int)),
